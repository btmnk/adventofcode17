phrases = """nyot babgr babgr kqtu kqtu kzshonp ylyk psqk
...
validPhrases = len(phraseArray)

print "### Total Phrases: ", validPhrases
...
for index in range(len(phraseArray)):
	currentPhrase = phraseArray[index]
	wordCheck = []
	words = currentPhrase.split(' ')
	for wordIndex in range(len(words)):
		currentWord = words[wordIndex]
		wordAlreadyUsed = currentWord in wordCheck
		anagramExists = 0
		for checkIndex in range(len(wordCheck)):
			checkWord = wordCheck[checkIndex]
			if anagramCheck(currentWord, checkWord):
				anagramExists = 1

		# don't check for anagram for part 1
		if wordAlreadyUsed or anagramExists:
			validPhrases -= 1
			break
		else:
			wordCheck.append(currentWord)
